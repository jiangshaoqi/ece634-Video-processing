...
def analyzeVideo(video_file, width, height, block_size, search_range):
    ...
    for i in range(num_frame):
        ...
        y_1 = cv2.resize(y_1, (y_1.shape[1]*2, y_1.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        u_1 = cv2.resize(u_1, (u_1.shape[1]*2, u_1.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        v_1 = cv2.resize(v_1, (v_1.shape[1]*2, v_1.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        ...
        y_2 = cv2.resize(y_2, (y_2.shape[1]*2, y_2.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        u_2 = cv2.resize(u_2, (u_2.shape[1]*2, u_2.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        v_2 = cv2.resize(v_2, (v_2.shape[1]*2, v_2.shape[0]*2), interpolation=cv2.INTER_LINEAR)
        ...
        for row in range(row_block_num):
            for col in range(col_block_num):
                ...
                search_row_end = (block_row_end + search_range) if (block_row_end + search_range < height * 2) else (height * 2 - 1)
                search_col_start = (block_col_start - search_range) if (block_col_start > search_range) else 0
                search_col_end = (block_col_end + search_range) if (block_col_end + search_range < width * 2) else (width * 2 - 1)
        ...
        field_img = np.zeros([height * 2, width * 2, 3], dtype=np.uint8)
        field_img.fill(255)
        for row in range(row_block_num):
            for col in range(col_block_num):
                ...
                field_img = cv2.arrowedLine(field_img, arrow_start, arrow_end, (0, 0, 255), 2)
        # rebuild the image
        re_y_1 = np.copy(y_1)
        re_u_1 = np.copy(u_1)
        re_v_1 = np.copy(v_1)
        ...
        bgr_1 = cv2.resize(bgr_1, (bgr_1.shape[1] // 2, bgr_1.shape[0] // 2))
        bgr_2 = cv2.resize(bgr_2, (bgr_2.shape[1] // 2, bgr_2.shape[0] // 2))
        field_img = cv2.resize(field_img, (field_img.shape[1] // 2, field_img.shape[0] // 2))
        re_bgr = cv2.resize(re_bgr, (re_bgr.shape[1] // 2, re_bgr.shape[0] // 2))
        # error image
        error_y = abs(np.array(re_y_1, dtype=float) - np.array(y_2, dtype=float))
        error_y = np.array(error_y, dtype=np.uint8)
        error_y = cv2.resize(error_y, (error_y.shape[1] // 2, error_y.shape[0] // 2))
        mse = (error_y ** 2).mean()
...
if __name__ == '__main__':
    # this program works for Part 2: half-pel accuracy
